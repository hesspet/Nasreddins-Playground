...
$projectFile = Join-Path $projectRoot "TestSherpa.csproj"
...
# Ports - abgestimmt mit Properties\launchSettings.json IIS Express
$httpPort  = 54817
$httpsPort = 44382
...
Write-Host "Veroeffentliche TestSherpa fuer IIS Express ..."
...
# Alle vorhandenen Sites entfernen, nur TestSherpa wird neu angelegt
$sitesToRemove = $sites.site | Where-Object { $_.name -ne "TestSherpa" }
foreach ($s in $sitesToRemove) { [void]$sites.RemoveChild($s) }
$existing = $sites.site | Where-Object { $_.name -eq "TestSherpa" }
...
if ($isAdmin) {
    ...
    $firewallRules = @(
        @{ Name = "TestSherpa-HTTP-$httpPort"; Port = $httpPort; Protocol = "TCP" },
        @{ Name = "TestSherpa-HTTPS-$httpsPort"; Port = $httpsPort; Protocol = "TCP" }
    )
...
$site.SetAttribute("name", "TestSherpa")
...
$iisArgs = @("/config:$workConfig", "/site:TestSherpa", "/systray:false")
