...
$projectFile = Join-Path $projectRoot "TestVosk.csproj"
...
Write-Host "Veroeffentliche TestVosk fuer IIS Express ..."
...
# Alle vorhandenen Sites entfernen, nur TestVosk wird neu angelegt
$sitesToRemove = $sites.site | Where-Object { $_.name -ne "TestVosk" }
foreach ($s in $sitesToRemove) { [void]$sites.RemoveChild($s) }
$existing = $sites.site | Where-Object { $_.name -eq "TestVosk" }
...
if ($isAdmin) {
    ...
    $firewallRules = @(
        @{ Name = "TestVosk-HTTP-$httpPort"; Port = $httpPort; Protocol = "TCP" },
        @{ Name = "TestVosk-HTTPS-$httpsPort"; Port = $httpsPort; Protocol = "TCP" }
    )
...
$site.SetAttribute("name", "TestVosk")
...
$iisArgs = @("/config:$workConfig", "/site:TestVosk", "/systray:false")
